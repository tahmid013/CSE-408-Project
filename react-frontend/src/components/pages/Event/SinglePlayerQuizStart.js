...
export default function SinglePlayerQuizStart() {
      ...
      if (secondsRemaining > 0) {
        ...
        const op_added = AddQuizTaken(
          localStorage.getItem(
            JSON.parse(localStorage.getItem("quizz-user")).user.id,
            id,
            cur_point
          )
        );
  ...
  useLayoutEffect(() => {
    const getData = async () => {
      ...
      setSecondsRemaining(quesList.length * 10);
      setQuesCount(1);
      setLoading(false);
    };
  ...
  const optClickChange = (param) => (e) => {
    ...
    if(param =="skipped"){
      ...
      console.log(
        "current point " +
          cur_point +
          " ques no " +
          ques_list[cur_ques_count - 1].point
      );
    ...
    if (cur_ques_count < total_ques_count) {
      ...
      localStorage.setItem("ques_op_list", JSON.stringify(op_list));
      localStorage.setItem("ques_choices", JSON.stringify(choices));
      const op_added = AddQuizTaken(authData.user.id, id, temp_point);
      console.log(
        "Final point " +
          cur_point +
          " ques no " +
  ...
  return (
    <div className="content-container">
      <div className="quiz-type-heading">
        <b> {id} MCQ Single Player</b>
      </div>
      ...
      <div className="question">
        {cur_ques_count}. {ques_list && ques_list[cur_ques_count - 1].question}
      </div>
      <div className="option-container">
        ...
        >
          {each_option_list && each_option_list.op_1}
        </div>
        <div
          className="option"
          onClick={optClickChange(each_option_list && each_option_list.op_2)}
        >
          {each_option_list && each_option_list.op_2}
        </div>
        <div
          className="option"
          onClick={optClickChange(each_option_list && each_option_list.op_3)}
        >
          {each_option_list && each_option_list.op_3}
        </div>
        <div
          className="option"
          onClick={optClickChange(each_option_list && each_option_list.op_4)}
        >
          {each_option_list && each_option_list.op_4}
        </div>
      </div>
      ...
      <div className="footer-container">
        <div
          className="option"
          onClick={optClickChange("skipped")}
        >
          skip
        </div>
